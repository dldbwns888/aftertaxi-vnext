...
class DistributionReport:
    ...
    # 리스크
    failure_prob: float       # mult_after_tax < 1.0 비율
    cvar_5pct: float          # worst 5% 평균 배수 (after-tax)
    median_mdd: float
    p95_mdd: float            # worst 5th percentile MDD
...
def run_lane_c(
    ...
    for path in paths:
        result = _run_single_path(path, config, base_fx_rate, source_fx_returns is not None)
        mult_pre.append(result.mult_pre_tax)
        mult_post.append(result.mult_after_tax)
        mdds.append(result.mdd)
        tax_drags.append(result.tax_drag)

    mult_pre = np.array(mult_pre)
    mult_post = np.array(mult_post)
    mdds = np.array(mdds)
    tax_drags = np.array(tax_drags)
    ...
    return DistributionReport(
        ...
        p95_mdd=float(np.percentile(mdds, 5)),  # 5th percentile = worst
        # tax
