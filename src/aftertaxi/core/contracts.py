...
class TaxConfig:
    ...
    # 종합과세 누진 (optional)
    # None이면 flat rate 유지. 튜플 리스트: [(구간상한_krw, 세율), ...]
    progressive_brackets: tuple = None
    progressive_threshold: float = 20_000_000.0  # 이 이상이면 누진 적용


# ── 한국 종합소득세 누진구간 (2024 기준, 지방세 포함) ──
# (구간 상한, 세율) — 지방세 10% 포함
KOREA_PROGRESSIVE_BRACKETS = (
    (14_000_000, 0.066),      # ~1,400만: 6% + 지방세 0.6%
    (50_000_000, 0.165),      # ~5,000만: 15% + 1.5%
    (88_000_000, 0.264),      # ~8,800만: 24% + 2.4%
    (150_000_000, 0.385),     # ~1.5억: 35% + 3.5%
    (300_000_000, 0.418),     # ~3억: 38% + 3.8%
    (500_000_000, 0.440),     # ~5억: 40% + 4.0%
    (1_000_000_000, 0.462),   # ~10억: 42% + 4.2%
    (float("inf"), 0.495),    # 10억 초과: 45% + 4.5%
)
...
class AccountConfig:
    ...
    annual_cap: Optional[float] = None  # 연간 납입 한도 (KRW). None이면 무제한. ISA=₩20,000,000
    lot_method: Literal["AVGCOST"] = "AVGCOST"  # AVGCOST 전용. FIFO/HIFO는 scope outside.
    ...
    def __post_init__(self):
        ...
        # annual_cap은 KRW, monthly_contribution은 USD이므로 직접 비교 불가.
        # 런타임에 allocation planner가 KRW 기준으로 cap 체크.
